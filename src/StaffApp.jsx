...
function StaffApp({ staffUser, lang, setLang, onLogout }) {
      ...
      if (salonProfile.loyalty_enabled) {
        ...
        supabase.functions.invoke("loyalty-notify", { body: { client_email: mail || null } }).then(({ data }) => {
          ...
          if (hit) toast.show(lang === "nl"
            ? `Stempelkaart${hit.staff_name ? ` bij ${hit.staff_name}` : ""} vol: ${hit.client_name || "Klant"} krijgt ${hit.pct}% korting — code is gemaild`
            : lang === "es"
  ...
  const openBlockEdit = (b) => {
    setBlockEditId(b.id);
    // Rijen zonder tijden (hele-dag- of dienst-blokkades) openen in dag-modus
    // zodat opslaan er geen tijdvak aan vastplakt.
    setBlockForm({ mode: b.block_time_start ? "time" : "day", variant: b.service_id ? "service" : "generic", from: b.date, to: "", time_start: b.block_time_start || "09:00", time_end: b.block_time_end || "17:00", reason: b.reason || "", service_id: b.service_id || "", repeat: b.weekday != null });
  ...
  const saveStaffBlock = async () => {
    ...
    // Behandeling-variant zonder keuze zou stilletjes alles blokkeren.
    ...
    try {
      ...
      // Wekelijkse herhaling: weekday van de gekozen datum; NULL = eenmalig.
      const repeatWeekday = blockForm.repeat && blockForm.from ? parseDate(blockForm.from).getDay() : null;
      // Editing an existing time-block row → UPDATE in place.
      if (blockEditId) {
        // Dag-modus (rij zonder tijden) mag bij bewerken geen tijdvak krijgen.
        const { data: updated, error } = await supabase.from("staff_day_overrides")
          .update({ date: blockForm.from, block_time_start: blockForm.mode === "time" ? blockForm.time_start : null, block_time_end: blockForm.mode === "time" ? blockForm.time_end : null, reason, service_id: blockForm.variant === "service" ? (blockForm.service_id || null) : null, weekday: repeatWeekday })
      ...
      const rows = [];
      // Dienst-specifiek ("maandag geen brows, pedicures wél"): alleen in de
      // behandeling-variant; de gewone blokkade raakt altijd de hele agenda.
      const svcId = blockForm.variant === "service" ? (blockForm.service_id || null) : null;
      if (blockForm.mode === "time") {
        rows.push({
          owner_id: salonProfile.id, staff_id: staffMember.id, date: blockForm.from,
          service_id: svcId, weekday: repeatWeekday,
          block_time_start: blockForm.time_start, block_time_end: blockForm.time_end, reason
        });
      } else if (repeatWeekday != null) {
        // Wekelijks: één rij (date = anker, weekday = dag) — geen losse datums.
        rows.push({
          owner_id: salonProfile.id, staff_id: staffMember.id, date: blockForm.from,
          service_id: svcId, weekday: repeatWeekday,
  ...
  const removeStaffBlock = async (id) => {
    const rij = staffBlocks.find(x => x.id === id);
    const weekly = rij && rij.weekday != null;
    if (!await showConfirm(weekly
      ? (lang === "nl" ? "Dit is een wekelijkse blokkade — verwijderen stopt hem voor álle komende weken. Doorgaan?" : lang === "es" ? "Este bloqueo es semanal — al quitarlo desaparece para todas las semanas. ¿Continuar?" : "This is a weekly block — removing it stops it for all future weeks. Continue?")
      : (lang === "nl" ? "Blokkade verwijderen?" : lang === "es" ? "¿Quitar el bloqueo?" : "Remove block?"))) return;
          ...
          {view === "agenda" && (() => {
            ...
            <div className="fade-up">
                ...
                return (
                  <div className="vl-card" data-week-grid style={{ marginBottom: 20, overflow: "hidden", display: "grid", gridTemplateColumns: "repeat(7, minmax(0, 1fr))" }}>
                    ...
                    {weekDays.map((d, i) => {
                      ...
                      const dayBlocks = staffBlocks.filter(b => blockAppliesOn(b, ds));
              ...
              {calViewMode !== "year" && staffBlocks
                .filter(b => blockAppliesOn(b, calDate))
                .map(b => {
                  ...
                  {visible.map(a => {
                    ...
                    return (
                      <div key={a.id} data-invoice-row style={{
                        display: "flex", alignItems: "center", flexShrink: 0, gap: 14,
          ...
          <div onClick={() => { if (!blockSaving) { setBlockModalOpen(false); setBlockEditId(null); } }}
            ...
            <div onClick={e => e.stopPropagation()}
              ...
              <div style={{ fontSize: 12, color: c.textSub, marginBottom: 18 }}>
                {blockForm.variant === "service"
                  ? (lang === "nl"
                    ? "Eén behandeling is dan tijdelijk niet bij jou boekbaar — de rest van je agenda gaat gewoon door."
                    : lang === "es"
                    ? "Un tratamiento deja de ser reservable contigo temporalmente — el resto de tu agenda sigue abierta."
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 18 }}>
                ...
                {blockForm.variant === "service" && (services || []).length > 0 && (
