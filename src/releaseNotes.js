...
export const RELEASES = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        text: {
          nl: "Vooruitbetalen per teamlid: heeft een teamlid eigen betaalgegevens (jij vult ze in bij Instellingen, Team, Bewerk, of zij in haar eigen app), dan gaat een vooruitbetaling voor een afspraak bij haar naar haar rekening. De boekingspagina, de bevestigingsmail, de herinnering en de WhatsApp-knop noemen dan haar IBAN of betaallink. Bij twee stylistes op een boeking, of zonder eigen gegevens, blijft het de rekening van de salon.",
          en: "Prepayment per team member: if a team member has her own payment details (you fill them in under Settings, Team, Edit, or she does in her own app), a prepayment for an appointment with her goes to her account. The booking page, the confirmation email, the reminder and the WhatsApp button then show her IBAN or payment link. With two stylists on one booking, or without her own details, it stays the salon's account.",
          es: "Pago por adelantado por miembro del equipo: si un miembro del equipo tiene sus propios datos de pago (los rellenas tú en Ajustes, Equipo, Editar, o ella en su app), el pago por adelantado de una cita con ella va a su cuenta. La página de reservas, el correo de confirmación, el recordatorio y el botón de WhatsApp muestran entonces su IBAN o enlace de pago. Con dos estilistas en una reserva, o sin datos propios, sigue siendo la cuenta del salón.",
        },
      },
      {
    ...
    items: [
      {
        kind: "new", audience: ["owner"],
        text: {
          nl: "Kasboek in de Kassa, onder het dagoverzicht: zet aan het begin van de dag het beginsaldo (Vellu stelt de laatste telling voor), boek kas in en kas uit met een reden (wisselgeld gehaald, bloemen, naar de bank), en tel aan het eind de la. Contante verkopen tellen vanzelf mee: wat de klant geeft staat als kas in, het wisselgeld als kas uit. Je ziet wat er in de la hoort te zitten en na het tellen het kasverschil. Per dag terug te bladeren, en te exporteren als PDF of Excel per dag, maand, kwartaal of jaar: per dag een regel met beginsaldo, contant ontvangen, wisselgeld, contant verkocht (netto), stortingen, opnames, verwacht, geteld en verschil, plus alle mutaties en contante betalingen.",
          en: "Cash book in Sales, below the day overview: set the opening float at the start of the day (Vellu suggests the last count), log cash in and cash out with a reason (change from the bank, flowers, to the bank), and count the drawer at the end. Cash sales are included automatically: what the client hands you shows as cash in, the change as cash out. You see what should be in the drawer and, after counting, the difference. Browse back per day, and export as PDF or Excel per day, month, quarter or year: one line per day with opening float, cash received, change, cash sales (net), deposits, withdrawals, expected, counted and difference, plus every movement and cash payment.",
          es: "Libro de caja en la Caja, debajo del resumen del día: fija el saldo inicial al empezar (Vellu sugiere el último recuento), registra entradas y salidas con un motivo (cambio del banco, flores, al banco) y cuenta la caja al final. Las ventas en efectivo se incluyen solas: lo que entrega el cliente aparece como entrada, el cambio como salida. Ves lo que debería haber en la caja y, tras contar, la diferencia. Se puede consultar por día y exportar en PDF o Excel por día, mes, trimestre o año: una línea por día con saldo inicial, efectivo recibido, cambio, ventas en efectivo (neto), depósitos, retiradas, esperado, contado y diferencia, más todos los movimientos y pagos en efectivo.",
        },
      },
      {
        kind: "new", audience: ["owner"],
