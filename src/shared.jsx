...
// "Nu" op de klok van de SALON, waar de bezoeker ook zit — hergebruikt de
// bestaande TZ_BY_COUNTRY/tzFor hierboven. Een bezoeker op Curaçao zag een
// Rijswijkse salon om 20:57 NL-tijd nog "Open · sluit 16:00" staan, en
// vandaag-sloten klopten in beide richtingen niet (29-08). De
...
export const salonNow = (countryCode) => {
  try { return new Date(new Date().toLocaleString("en-US", { timeZone: tzFor(countryCode) })); }
...
const _T_RAW = {
  nl: {
    ...
    analytics:"Analytics", weeklyRevenue:"Omzet deze week", monthlyRevenue:"Omzet deze maand", yearlyRevenue:"Omzet dit jaar",
    ...
    bookingError:"Er ging iets mis bij het boeken. Ververs de pagina en probeer het opnieuw.",
    ...
    reviewAnonymous:"Anoniem plaatsen — je naam wordt niet getoond", anonymousClient:"Anoniem",
    reviewRequestIntro:"Reviews komen alleen van klanten die echt geweest zijn. Vul het e-mailadres in waarmee je hebt geboekt, dan sturen we je je persoonlijke reviewlink.",
    reviewRequestEmail:"Je e-mailadres", reviewRequestSend:"Stuur mijn reviewlink",
    reviewRequestSent:"Als we een afspraak op dat adres vinden, staat je link binnen een paar minuten in je inbox (kijk ook bij spam).",
    reviewRequestFailed:"Versturen mislukt — probeer het zo nog eens.",
  ...
  en: {
    ...
    analytics:"Analytics", weeklyRevenue:"Revenue this week", monthlyRevenue:"Revenue this month", yearlyRevenue:"Revenue this year",
    ...
    bookingError:"Something went wrong while booking. Please refresh the page and try again.",
    ...
    reviewAnonymous:"Post anonymously — your name won't be shown", anonymousClient:"Anonymous",
    reviewRequestIntro:"Reviews only come from clients who have actually visited. Enter the email address you booked with and we'll send you your personal review link.",
    reviewRequestEmail:"Your email address", reviewRequestSend:"Send my review link",
    reviewRequestSent:"If we find an appointment on that address, your link will be in your inbox within a few minutes (check spam too).",
    reviewRequestFailed:"Sending failed — please try again in a moment.",
  ...
  es: {
    ...
    analytics:"Estadísticas", weeklyRevenue:"Ingresos de esta semana", monthlyRevenue:"Ingresos de este mes", yearlyRevenue:"Ingresos de este año",
    ...
    bookingError:"Algo salió mal al reservar. Recarga la página e inténtalo de nuevo.",
    ...
    reviewAnonymous:"Publicar de forma anónima — no se mostrará tu nombre", anonymousClient:"Anónimo",
    reviewRequestIntro:"Las reseñas solo vienen de clientes que realmente han venido. Introduce el correo con el que reservaste y te enviaremos tu enlace personal para dejar una reseña.",
    reviewRequestEmail:"Tu correo electrónico", reviewRequestSend:"Enviar mi enlace",
    reviewRequestSent:"Si encontramos una cita con ese correo, tu enlace llegará a tu bandeja en unos minutos (mira también en spam).",
    reviewRequestFailed:"No se pudo enviar — inténtalo de nuevo en un momento.",
...
// Randkleur voor GEVULDE accentvlakken (knoppen/chips) die anders in de
// achtergrond verdwijnen: wit accent op licht thema krijgt een zachte donkere
// outline (Faisals "witte pillen met randje", 27-08), zwart accent op donker
// thema een lichte. Al het andere: "transparent" (geen zichtbare rand).
export const accentEdge = (raw, themeName) => {
  const L = _hexLum(_sanitizeAccent(raw));
  if (themeName === "light" && L > 0.72) return "rgba(0,0,0,0.28)";
...
// automatische outline (accentEdge) zodat ze niet in de achtergrond wegvallen.
const makeCSS = (rawAccent, c = THEMES.dark, surfaceRaw = rawAccent, themeName = null) => {
  const accent = _sanitizeAccent(rawAccent);
  const onAccent = onAccentInk(accent, c.btnOnDark);
  const surface = _sanitizeAccent(surfaceRaw);
  const surfaceInk = onAccentInk(surface, c.btnOnDark);
  const edge = accentEdge(surface, themeName || (c === THEMES.light ? "light" : "dark"));
  // Voor rules die al een border hebben: randkleur = outline of gewoon de vulkleur.
  const surfaceBorder = edge === "transparent" ? surface : edge;
  return `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .time-chip.sel { background: ${surface}; border-color: ${surfaceBorder}; color: ${surfaceInk}; font-weight: 600; }
  ...
  .profile-team-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; align-items: stretch; }
  .profile-team-card {
  ...
  /* Voet: chips op één regel (afgekapt, hele lijst in de title-tooltip) en
  ...
  .profile-review-cards { flex: 1; min-width: 0; display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; align-items: start; }
  .profile-review-card {
  ...
  /* minmax(0, 1fr): een lange omschrijving op één regel mag de kolommen
     nooit breder duwen dan de pagina (My Whims, 15-09). */
  .profile-products-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; }
  ...
  .profile-contact-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; margin-bottom: 14px; }
  .profile-contact-card { min-width: 0; overflow-wrap: anywhere; }
  ...
  .profile-cat-pill.active {
    background: ${surface}; color: ${surfaceInk}; border-color: ${surfaceBorder}; font-weight: 600;
  }
...
function Layout({ children, accent = ACCENT, rawAccent = null, maxWidth = "100%" }) {
  ...
  const css = useMemo(() => makeCSS(accent, c, rawAccent || accent, theme), [accent, c, rawAccent, theme]);
  return (
