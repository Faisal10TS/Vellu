...
const reasonRow=eR?`<tr><td ${cL}>${txt(lang,"Reden","Reason","Motivo")}</td><td ${cR}>${eR}</td></tr>`:"";
const subj=txt(lang,`Afspraak geannuleerd: ${b.client_name}`,`Appointment cancelled: ${b.client_name}`,`Cita cancelada: ${b.client_name}`);
for(const em of rcp){await send(plainText(em),plainText(subj),`${W}${lH(b)}<h2 style="font-weight:400;font-size:22px;margin-bottom:8px;color:#dc2626;">${txt(lang,"Afspraak geannuleerd","Appointment cancelled","Cita cancelada")}</h2><p style="color:#666;margin-bottom:28px;">${txt(lang,`<strong>${eC}</strong> heeft de afspraak bij <strong>${eS}</strong> geannuleerd. Deze tijd is nu weer vrij in je agenda.`,`<strong>${eC}</strong> cancelled their appointment at <strong>${eS}</strong>. This slot is now free again in your agenda.`,`<strong>${eC}</strong> canceló su cita en <strong>${eS}</strong>. Este horario vuelve a estar libre en tu agenda.`)}</p><div ${bS.replace('margin-bottom:28px;','')}><table ${tS}>${row(txt(lang,"Klant","Client","Cliente"),eC)}${b.client_phone?row(txt(lang,"Telefoon","Phone","Teléfono"),ePh):""}${row(txt(lang,"Behandeling","Treatment","Servicio"),eSv)}${row(txt(lang,"Datum","Date","Fecha"),esc(fmtD(b.date,lang)))}${row(txt(lang,"Tijd","Time","Hora"),eT)}${reasonRow}</table></div></div>`);}}
...
if(b.salon_btw&&rate>0){const net=gross/(1+rate);const vat=gross-net;const rl=parseFloat((rate*100).toFixed(2)).toString().replace(".",lang==="nl"?",":".");vatRows=`${row(`${txt(lang,"Subtotaal","Subtotal","Subtotal")} (${txt(lang,"excl.","excl.","sin")} ${TAXL})`,fP(net))}${row(`${TAXL} ${rl}%`,fP(vat))}`;}
const totLabel=b.salon_btw?`${txt(lang,"Totaal","Total","Total")} (${txt(lang,"incl.","incl.","con")} ${TAXL})`:txt(lang,"Totaal","Total","Total");
const noVatNote=b.salon_btw?"":`<p style="color:#aaa;font-size:11px;text-align:center;margin:0 0 8px;">${txt(lang,"Geen belasting in rekening gebracht.","No tax charged.","No se aplican impuestos.")}</p>`;
...
await send(plainText(b.client_email),plainText(txt(lang,`Herinnering: Morgen afspraak bij ${b.salon_name}`,`Reminder: Appointment tomorrow at ${b.salon_name}`,`Recordatorio: Mañana tienes cita en ${b.salon_name}`)),`${W}${lH(b)}<h2 style="font-weight:400;font-size:22px;margin-bottom:8px;">${txt(lang,"Niet vergeten!","Don't forget!","¡No lo olvides!")}</h2><p style="color:#666;margin-bottom:28px;">${txt(lang,`Je hebt morgen een afspraak bij <strong>${eS}</strong>`,`You have an appointment tomorrow at <strong>${eS}</strong>`,`Mañana tienes una cita en <strong>${eS}</strong>`)}</p><div ${bS}><table ${tS}>${row(txt(lang,"Behandeling","Treatment","Servicio"),eSv)}${row(txt(lang,"Datum","Date","Fecha"),eD)}${row(txt(lang,"Tijd","Time","Hora"),eT)}${totRow(txt(lang,"Totaal","Total","Total"),fP(b.price))}</table></div><p style="color:#888;font-size:13px;text-align:center;">${txt(lang,`We zien je morgen, ${eC}!`,`See you tomorrow, ${eC}!`,`¡Nos vemos mañana, ${eC}!`)}</p>${b.salon_slug?`<p style="text-align:center;margin-top:20px;"><a href="https://vellu.cc/${eSl}" style="color:${AC};text-decoration:none;font-size:12px;">vellu.cc/${eSl}</a></p>`:""}</div>`);}
...
const subj=txt(lang,`Nieuwe wachtlijst-aanmelding: ${b.client_name}`,`New waitlist request: ${b.client_name}`,`Nueva solicitud de lista de espera: ${b.client_name}`);
for(const em of rcp){await send(plainText(em),plainText(subj),`${W}${lH(b)}<h2 style="font-weight:400;font-size:22px;margin-bottom:8px;">${txt(lang,"Nieuwe wachtlijst-aanmelding","New waitlist request","Nueva solicitud de lista de espera")}</h2><p style="color:#666;margin-bottom:28px;">${txt(lang,`<strong>${eC}</strong> wil op de wachtlijst bij <strong>${eS}</strong>.`,`<strong>${eC}</strong> wants to join the waitlist at <strong>${eS}</strong>.`,`<strong>${eC}</strong> quiere unirse a la lista de espera de <strong>${eS}</strong>.`)}</p><div ${bS.replace('margin-bottom:28px;','')}><table ${tS}>${row(txt(lang,"Klant","Client","Cliente"),eC)}${row(txt(lang,"E-mail","Email","Correo"),eEm)}${b.client_phone?row(txt(lang,"Telefoon","Phone","Teléfono"),ePh):""}${b.service_name?row(txt(lang,"Behandeling","Treatment","Servicio"),eSv):""}${b.staff_name?row(txt(lang,"Medewerker","Staff","Personal"),esc(b.staff_name)):""}${row(txt(lang,ds.length===1?"Gewenste dag":"Gewenste dagen",ds.length===1?"Preferred day":"Preferred days",ds.length===1?"Día preferido":"Días preferidos"),daysStr)}${eNo?`<tr><td ${cL}>${txt(lang,"Notitie","Note","Nota")}</td><td ${cR}>${eNo}</td></tr>`:""}</table></div></div>`);}}
