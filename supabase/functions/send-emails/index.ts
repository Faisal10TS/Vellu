...
// Te veel betaald en teruggestort door de salon (prijs omlaag na een
// vooruitbetaling). Verstuurd vanuit de app bij "Terugbetaald" op de kaart.
if(type==="refund_sent"){
const refund=parseFloat(b.refund||0)||0;const paidAmt=parseFloat(b.amount_paid||0)||0;
await send(plainText(b.client_email),plainText(txt(lang,`Terugbetaling van ${fP(refund)} door ${b.salon_name}`,`Refund of ${fP(refund)} from ${b.salon_name}`,`Devolución de ${fP(refund)} de ${b.salon_name}`)),`${W}${lH(b)}<h2 style="font-weight:400;font-size:22px;margin-bottom:8px;">${txt(lang,"Terugbetaling","Refund","Devolución")}</h2><p style="color:#666;margin-bottom:28px;">${txt(lang,`<strong>${eS}</strong> heeft <strong>${fP(refund)}</strong> aan je teruggestort: je had ${fP(paidAmt)} vooruitbetaald en je behandeling kost ${fP(b.price)}. Het bedrag staat meestal binnen één tot twee werkdagen op je rekening.`,`<strong>${eS}</strong> has refunded <strong>${fP(refund)}</strong> to you: you paid ${fP(paidAmt)} in advance and your treatment costs ${fP(b.price)}. The amount usually arrives within one or two working days.`,`<strong>${eS}</strong> te ha devuelto <strong>${fP(refund)}</strong>: pagaste ${fP(paidAmt)} por adelantado y tu tratamiento cuesta ${fP(b.price)}. El importe suele llegar en uno o dos días laborables.`)}</p><div ${bS}><table ${tS}>${row(txt(lang,"Behandeling","Treatment","Servicio"),eSv)}${row(txt(lang,"Datum","Date","Fecha"),eD)}${b.time?row(txt(lang,"Tijd","Time","Hora"),eT):""}${row(txt(lang,"Vooruitbetaald","Paid in advance","Pagado por adelantado"),fP(paidAmt))}${row(txt(lang,"Prijs behandeling","Treatment price","Precio del tratamiento"),fP(b.price))}${totRow(txt(lang,"Teruggestort","Refunded","Devuelto"),fP(refund))}</table></div><p style="color:#888;font-size:13px;text-align:center;">${txt(lang,`Tot de volgende keer, ${eC}!`,`See you next time, ${eC}!`,`¡Hasta la próxima, ${eC}!`)}</p></div>`);}
...
// Al (vooruit)betaald? Dan is dit meteen het betaalverzoek voor het verschil:
// "Al betaald -€45 / Nog te betalen €30" + betaalblok voor €30. Lagere prijs
// dan betaald → geen verzoek maar een terugbetaalregel.
const paidAmt=Math.max(0,parseFloat(b.amount_paid||0)||0);const openAmt=Math.round((parseFloat(b.price||0)-paidAmt)*100)/100;
const payRows=paidAmt>0?`${row(txt(lang,"Al betaald","Already paid","Ya pagado"),`-${fP(paidAmt)}`)}${openAmt<-0.005?row(txt(lang,"Te veel betaald, de salon betaalt dit terug","Overpaid, the salon will refund this","Pagado de más, el salón lo devolverá"),fP(-openAmt)):totRow(txt(lang,"Nog te betalen","Still to pay","Pendiente de pago"),fP(Math.max(0,openAmt)))}`:"";
const payReq=paidAmt>0&&openAmt>0.005?`<p style="color:#666;font-size:13px;margin:0 0 12px;">${txt(lang,`Je had al ${fP(paidAmt)} vooruitbetaald. Het verschil van <strong>${fP(openAmt)}</strong> kun je hieronder overmaken, of in de salon betalen.`,`You already paid ${fP(paidAmt)} in advance. You can transfer the difference of <strong>${fP(openAmt)}</strong> below, or pay it in the salon.`,`Ya pagaste ${fP(paidAmt)} por adelantado. Puedes transferir la diferencia de <strong>${fP(openAmt)}</strong> aquí abajo, o pagarla en el salón.`)}</p>${payBlockHtml(openAmt,String(b.payment_ref||`${b.salon_name||"Vellu"} ${b.date||""}`).slice(0,100))}`:"";
await send(plainText(b.client_email),plainText(txt(lang,`Wijziging afspraak bij ${b.salon_name}`,`Appointment updated at ${b.salon_name}`,`Cita modificada en ${b.salon_name}`)),`${W}${lH(b)}<h2 style="font-weight:400;font-size:22px;margin-bottom:8px;">${txt(lang,"Je afspraak is gewijzigd","Your appointment has been updated","Tu cita ha sido modificada")}</h2><p style="color:#666;margin-bottom:28px;">${txt(lang,`<strong>${eS}</strong> heeft de details van je afspraak aangepast. Hieronder zie je de nieuwe gegevens:`,`<strong>${eS}</strong> updated the details of your appointment. The new details are below:`,`<strong>${eS}</strong> actualizó los detalles de tu cita. A continuación puedes ver los nuevos datos:`)}</p><div ${bS}><table ${tS}>${row(txt(lang,"Behandeling","Treatment","Servicio"),eSv)}${changedRow(txt(lang,"Datum","Date","Fecha"),oldDate,eD)}${changedRow(txt(lang,"Tijd","Time","Hora"),oldTime,eT)}${changedRow(txt(lang,"Totaal","Total","Total"),oldPrice,newPrice)}${payRows}</table></div>${payReq}${cs}<p style="color:#888;font-size:13px;text-align:center;">${txt(lang,`Zien we je dan, ${eC}!`,`See you then, ${eC}!`,`¡Nos vemos entonces, ${eC}!`)}</p></div>`);}
if(type==="invoice"){
