...
async function sendEmail(to,subject,html){if(!R)throw new Error("RESEND_API_KEY not configured");const res=await fetch("https://api.resend.com/emails",{method:"POST",headers:{"Authorization":`Bearer ${R}`,"Content-Type":"application/json"},body:JSON.stringify({from:F,to,subject,html})});const data=await res.json();if(!res.ok){console.error("Resend API error:",data);throw new Error(data?.message||`Email send failed (${res.status})`);}return data;}
...
const lH=(b)=>{const logo=safeImgSrc(b.salon_logo);const n=esc(b.salon_name);if(logo)return`<div style="text-align:center;margin-bottom:32px;"><img src="${esc(logo)}" alt="${n}" style="max-height:60px;max-width:200px;margin-bottom:12px;" /><div style="width:40px;height:1px;background:#c9a96e;margin:0 auto;"></div></div>`;return`<div style="text-align:center;margin-bottom:32px;"><h1 style="font-size:32px;font-weight:300;letter-spacing:0.1em;margin:0;">vellu</h1><div style="width:40px;height:1px;background:#c9a96e;margin:12px auto;"></div></div>`;};
...
const totRow=(l,r)=>`<tr ${gL}><td ${gA}>${l}</td><td ${gR}>${r}</td></tr>`;
...
await sendEmail(plainText(b.client_email),plainText(txt(lang,`Bevestiging afspraak bij ${b.salon_name}`,`Appointment confirmed at ${b.salon_name}`)),`${W}${lH(b)}<h2 style="font-weight:400;font-size:22px;margin-bottom:8px;">${txt(lang,"Je afspraak is bevestigd","Your appointment is confirmed")}</h2><p style="color:#666;margin-bottom:28px;">${txt(lang,`Bedankt voor je boeking bij <strong>${eS}</strong>`,`Thank you for booking at <strong>${eS}</strong>`)}</p><div ${bS}><table ${tS}>${row(txt(lang,"Behandeling","Treatment"),eSv)}${row(txt(lang,"Datum","Date"),eD)}${row(txt(lang,"Tijd","Time"),eT)}${row(txt(lang,"Betaling","Payment"),b.payment==="online"?txt(lang,"Online betaald","Paid online"):txt(lang,"Betalen bij afspraak","Pay at appointment"))}${totRow(txt(lang,"Totaal","Total"),fP(b.price))}</table></div>${cs}<p style="color:#888;font-size:13px;text-align:center;">${txt(lang,`Tot dan, ${eC}!`,`See you then, ${eC}!`)}</p></div>`);}
...
if(type==="invoice"){
// Salon accent colour for branding. Validated to a 6-digit hex literal to
// prevent any CSS/HTML injection; falls back to Vellu gold.
const AC=/^#[0-9a-fA-F]{6}$/.test(String(b.salon_accent||""))?b.salon_accent:"#c9a96e";
// Header with the salon's accent divider (instead of the shared gold one).
const ilogo=safeImgSrc(b.salon_logo);
const lHa=ilogo?`<div style="text-align:center;margin-bottom:32px;"><img src="${esc(ilogo)}" alt="${eS}" style="max-height:60px;max-width:200px;margin-bottom:12px;" /><div style="width:40px;height:1px;background:${AC};margin:0 auto;"></div></div>`:`<div style="text-align:center;margin-bottom:32px;"><h1 style="font-size:32px;font-weight:300;letter-spacing:0.1em;margin:0;">vellu</h1><div style="width:40px;height:1px;background:${AC};margin:12px auto;"></div></div>`;
// Accent-coloured total row.
const totRowA=(l,r)=>`<tr ${gL}><td style="padding:12px 0 4px;font-weight:600;color:${AC};">${l}</td><td style="padding:12px 0 4px;font-weight:600;color:${AC};text-align:right;">${r}</td></tr>`;
...
await sendEmail(plainText(b.client_email),plainText(`${txt(lang,"Factuur","Invoice")} ${b.invoice_number||""} - ${b.salon_name}`),`${W}${lHa}<h2 style="font-weight:400;font-size:22px;margin:0 0 4px;">${txt(lang,"Factuur","Invoice")}</h2><p style="color:#888;font-size:13px;margin:0 0 24px;">${eS} &middot; ${eD}</p>${b.invoice_number?`<div style="background:${AC}1a;border-radius:8px;padding:8px 14px;font-size:13px;font-weight:600;color:${AC};display:inline-block;margin-bottom:16px;">${eIN}</div>`:""}${bSec}<div ${bS}><table ${tS}>${row(txt(lang,"Klant","Client"),eC)}${row(txt(lang,"Behandeling","Treatment"),eSv)}${row(txt(lang,"Datum","Date"),eD)}${vatRows}${totRowA(totLabel,fP(gross))}</table></div>${noVatNote}<p style="color:#888;font-size:12px;text-align:center;">${txt(lang,"Bedankt voor je bezoek!","Thank you for your visit!")}</p></div>`);}
if(type==="appointment_reminder"){
await sendEmail(plainText(b.client_email),plainText(txt(lang,`Herinnering: Morgen afspraak bij ${b.salon_name}`,`Reminder: Appointment tomorrow at ${b.salon_name}`)),`${W}${lH(b)}<h2 style="font-weight:400;font-size:22px;margin-bottom:8px;">${txt(lang,"Niet vergeten!","Don't forget!")}</h2><p style="color:#666;margin-bottom:28px;">${txt(lang,`Je hebt morgen een afspraak bij <strong>${eS}</strong>`,`You have an appointment tomorrow at <strong>${eS}</strong>`)}</p><div ${bS}><table ${tS}>${row(txt(lang,"Behandeling","Treatment"),eSv)}${row(txt(lang,"Datum","Date"),eD)}${row(txt(lang,"Tijd","Time"),eT)}${totRow(txt(lang,"Totaal","Total"),fP(b.price))}</table></div><p style="color:#888;font-size:13px;text-align:center;">${txt(lang,`We zien je morgen, ${eC}!`,`See you tomorrow, ${eC}!`)}</p>${b.salon_slug?`<p style="text-align:center;margin-top:20px;"><a href="https://vellu.cc/${eSl}" style="color:#c9a96e;text-decoration:none;font-size:12px;">vellu.cc/${eSl}</a></p>`:""}</div>`);}
